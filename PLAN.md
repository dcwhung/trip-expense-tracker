...
### 畫面 1 — 入數（開 app 預設）
由上至下：
1. **日期** — 預設今日，可改（方便第二朝補入琴晚嗰餐）
2. **Category** — 3×2 icon grid，一撳選中
3. **金額** — 大字，`inputmode="decimal"`，原生數字鍵盤
4. **Payment** — 三格 toggle（sticky）
5. **Account** — 兩格 toggle（sticky）
6. **Description** — optional，單行
7. **Remarks** — optional，多行
8. **儲存** — 固定喺底部，有回饋，表單清空，準備入下一筆

目標：**入一筆 ≤ 8 秒、≤ 3 下撳**（category → 金額 → 儲存；日期同兩個 toggle 平時唔使掂）。

**鍵盤處理**：金額排喺 category 之後，所以數字鍵盤通常係最後先彈出。iOS 嘅小數鍵盤冇「完成」掣，所以個儲存掣用 `visualViewport` 跟住鍵盤高度浮上去，唔會被蓋住。**呢個一定要喺真機驗證**（見 §6）。
...
- [ ] 金額打 `12.345` → 個欄自動變 `12.34`，打唔到第三個小數位；儲存後係 €12.34
